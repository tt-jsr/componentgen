...
#include "Core70Api/inc/SSCoreApiEventIDs.h"
#include "Core70Api/inc/PriceEvent.h"
#include "ssutils/inc/TimerEvent.h"
#include <sstream>
...
{
    if(!m_hTimer)
        m_hTimer = GetStrategy(STRATEGY_NAME_TIMER_STRATEGY);
    ...
    if (!m_hPrice)
        m_hPrice = GetStrategy(STRATEGY_NAME_PRICE_STRATEGY);

    if (!m_hLogger || !m_hConfig || !m_hPrice)
    {
        m_hTimer->QueueEvent(SSEventHandle(new TimerEvent(1000, GetThisHandle(), RQSTID_TIMER_STARTUP)));
    }
}
...
/* Handle requests */
void $(STRATNAME)Strategy::SOMHandleRequestEvent(unsigned long iThreadID, SSEventHandle& hEvent)
{
    // Look at the request
	switch ( hEvent->GetEventType() )
	{
...
void $(STRATNAME)Strategy::SOMHandleResponseEvent(unsigned long iThreadID, SSEventHandle& hEvent)
{
...
void $(STRATNAME)Strategy::SOMHandleNotifyEvent(unsigned long iThreadID, SSEventHandle& hEvent)
{
    return;
}

void $(STRATNAME)Strategy::HandleNativeFill(GeneralFillHandle& fill)
{

}

void $(STRATNAME)Strategy::HandleNativeOrderStatus(GeneralOrderStatus& status)
{
